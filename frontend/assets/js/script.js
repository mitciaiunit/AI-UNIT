...
const translations = {
  en: {
    nav_action: "AI in Action", nav_framework: "Framework", nav_about: "About Us", nav_contact: "Contact Us", search: "Search", accessibility: "Accessibility",
    ...
    cookie_text: "We use cookies to enhance your experience, analyse site traffic, and improve our services. By continuing to browse, you agree to our <a href=\"cookie.html\" target=\"_blank\" rel=\"noopener\">Cookie Policy</a>.",
...
document.getElementById('searchBtn').addEventListener('click',()=>{searchModal.classList.add('open');setTimeout(()=>searchInput.focus(),100);});
document.getElementById('searchClose').addEventListener('click',()=>searchModal.classList.remove('open'));
document.querySelector('.search-backdrop').addEventListener('click',()=>searchModal.classList.remove('open'));
...
document.getElementById('readEnBooklet')?.addEventListener('click', () => window.open('aie.html', '_blank'));
document.getElementById('readKmBooklet')?.addEventListener('click', () => window.open('aim.html', '_blank'));
...
(function(){
  ...
  function openVideoModal(src,title){
    ...
    if (trackEn) trackEn.src = 'caption/' + baseName + '-en.vtt';
    if (trackFr) trackFr.src = 'caption/' + baseName + '-fr.vtt';
    if (trackKm) trackKm.src = 'caption/' + baseName + '-km.vtt';
    modalVideo.src = src;
    modalVideo.load();
  ...
  setTimeout(function(){
    try{
      ...
      const SR = {
        ...
        rate: 1,
        // Language mapping:
        //   en → en-GB  (British English, consistent with DIVA)
        //   fr → fr-FR
        //   km → fr-FR  (Kreol Morisien: no native TTS, French is closest)
        langMap: { en: 'en-GB', fr: 'fr-FR', km: 'fr-FR' },
        // Preferred voices — same priority list as DIVA for consistency
        preferredVoices: {
          en: ['Google UK English Female', 'Google UK English Male', 'Microsoft Hazel - English (United Kingdom)', 'Microsoft George - English (United Kingdom)', 'Daniel'],
          fr: ['Google Français', 'Microsoft Julie - French (France)', 'Microsoft Hortense - French (France)', 'Thomas'],
          km: ['Google Français', 'Microsoft Julie - French (France)', 'Microsoft Hortense - French (France)', 'Thomas']
      ...
      function getReadableText() {
        const walker = document.createTreeWalker(
          ...
          function (node) {
            ...
            if (parent.closest && parent.closest('#a11y-panel, .search-modal, .video-modal, .diva-panel, #divaWidget, #a11y-announcer, #a11y-read-guide')) return NodeFilter.FILTER_REJECT;
      ...
      window.addEventListener('aiunit-lang-changed', function () {
        ...
        // Update video subtitle tracks based on new language
        const modalVideo = document.getElementById('modalVideo');
        if (modalVideo && modalVideo.textTracks) {
          const tracks = modalVideo.textTracks;
          for (let i = 0; i < tracks.length; i++) {
            const track = tracks[i];
            if (track.kind === 'subtitles') {
      ...
      const S={fontIdx:2,colorMode:'normal'};
      const fontSteps=[80,90,100,110,120,135,150];
      const STORAGE_KEY='a11y_govmu_prefs_v5';
      ...
      function openPanel(){panel.classList.add('open');trigger.setAttribute('aria-expanded','true');const diva=document.getElementById('divaWidget');if(diva)diva.classList.add('panel-open');announce('Accessibility panel opened');}
      function closePanel(){panel.classList.remove('open');trigger.setAttribute('aria-expanded','false');const diva=document.getElementById('divaWidget');if(diva)diva.classList.remove('panel-open');trigger.focus();}
      function togglePanel(){panel.classList.contains('open')?closePanel():openPanel();}
      trigger.addEventListener('click',togglePanel);
      trigger.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();togglePanel();}});
      document.getElementById('a11y-close-btn').addEventListener('click',closePanel);
      document.addEventListener('keydown',function(e){
        if(e.key==='Escape'&&panel.classList.contains('open')){closePanel();}
        if((e.altKey||e.metaKey)&&e.key.toLowerCase()==='a'){e.preventDefault();togglePanel();}
      });
      const fontInc=document.getElementById('font-inc');
      const fontDec=document.getElementById('font-dec');
      fontInc&&fontInc.addEventListener('click',()=>{S.fontIdx=Math.min(fontSteps.length-1,S.fontIdx+1);applyFont();});
      fontDec&&fontDec.addEventListener('click',()=>{S.fontIdx=Math.max(0,S.fontIdx-1);applyFont();});
      function applyFont(){const val=fontSteps[S.fontIdx];html.style.fontSize=val+'%';if(fontDisplay)fontDisplay.textContent=val+'%';announce('Text size '+val+'%');save();}
      const TOGGLES={
        't-dyslexia':{cls:'a11y-dyslexia',label:'Dyslexia font'},
        't-links':{cls:'a11y-highlight-links',label:'Link highlighting'},
        't-readguide':{cls:'a11y-read-guide',label:'Reading guide'},
        't-letterspacing':{cls:'a11y-letter-spacing',label:'Letter spacing'},
        't-cursor':{cls:'a11y-big-cursor',label:'Large cursor'},
        't-focus':{cls:'a11y-focus-visible',label:'Focus highlight'},
        't-images':{cls:'a11y-no-images',label:'Hide images'},
        't-motion':{cls:'a11y-no-motion',label:'Reduce motion'},
        't-keyboard':{cls:'a11y-keyboard',label:'Keyboard guide'},
      };
      Object.keys(TOGGLES).forEach(function(id){
        const toggle=document.getElementById(id);const row=document.getElementById('lbl-'+id);
        if(!toggle)return;
        toggle.addEventListener('change',function(){const isOn=toggle.checked;if(row)row.classList.toggle('on',isOn);toggle.setAttribute('aria-checked',isOn?'true':'false');html.classList.toggle(TOGGLES[id].cls,isOn);if(id==='t-readguide')readGuide.style.display=isOn?'block':'none';announce(TOGGLES[id].label+(isOn?' on':' off'));save();});
        if(row)row.addEventListener('click',function(e){if(e.target===toggle)return;toggle.checked=!toggle.checked;toggle.dispatchEvent(new Event('change'));});
      });
      document.addEventListener('mousemove',function(e){if(!html.classList.contains('a11y-read-guide'))return;readGuide.style.top=(e.clientY-18)+'px';});
      document.querySelectorAll('.a11y-color-opt').forEach(function(btn){
        btn.addEventListener('click',function(){
          const mode=btn.dataset.mode;const isAlreadyActive=btn.classList.contains('active');
          document.querySelectorAll('.a11y-color-opt').forEach(function(b){b.classList.remove('active');b.setAttribute('aria-pressed','false');});
          ['high-contrast','dark','grayscale','negative'].forEach(function(m){html.classList.remove('a11y-'+m);});
          if(isAlreadyActive){const normalBtn=document.querySelector('.a11y-color-opt[data-mode="normal"]');if(normalBtn){normalBtn.classList.add('active');normalBtn.setAttribute('aria-pressed','true');}S.colorMode='normal';announce('Colour mode reset to Normal');}
          else{btn.classList.add('active');btn.setAttribute('aria-pressed','true');if(mode!=='normal')html.classList.add('a11y-'+mode);S.colorMode=mode;announce('Colour mode: '+btn.textContent.trim());}
          save();
        });
      });
      const PROFILES={
        vision:{toggles:['t-focus','t-links'],fontIdx:5,colorMode:'high-contrast'},
        motor:{toggles:['t-cursor','t-keyboard'],fontIdx:3,colorMode:'normal'},
        dyslexia:{toggles:['t-dyslexia','t-letterspacing','t-readguide'],fontIdx:4,colorMode:'normal'},
        elderly:{toggles:['t-cursor','t-links','t-focus'],fontIdx:5,colorMode:'normal'},
        cognitive:{toggles:['t-motion','t-readguide','t-links'],fontIdx:3,colorMode:'normal'},
      };
      document.querySelectorAll('.a11y-profile').forEach(function(btn){
        btn.addEventListener('click',function(){
          const type=btn.dataset.profile;const profile=PROFILES[type];if(!profile)return;
          if(btn.classList.contains('active')){resetAll();return;}
          resetAll(true);
          profile.toggles.forEach(function(id){const toggle=document.getElementById(id);const row=document.getElementById('lbl-'+id);if(!toggle)return;toggle.checked=true;toggle.setAttribute('aria-checked','true');if(row)row.classList.add('on');html.classList.add(TOGGLES[id].cls);if(id==='t-readguide')readGuide.style.display='block';});
          S.fontIdx=profile.fontIdx;applyFont();
          const modeBtn=document.querySelector('.a11y-color-opt[data-mode="'+profile.colorMode+'"]');
          if(modeBtn){document.querySelectorAll('.a11y-color-opt').forEach(function(b){b.classList.remove('active');b.setAttribute('aria-pressed','false');});modeBtn.classList.add('active');modeBtn.setAttribute('aria-pressed','true');['high-contrast','dark','grayscale'].forEach(function(m){html.classList.remove('a11y-'+m);});if(profile.colorMode!=='normal')html.classList.add('a11y-'+profile.colorMode);S.colorMode=profile.colorMode;}
          document.querySelectorAll('.a11y-profile').forEach(function(b){b.classList.remove('active');b.setAttribute('aria-pressed','false');});
          btn.classList.add('active');btn.setAttribute('aria-pressed','true');
          announce(type+' profile applied');save();
        });
      });
      document.getElementById('a11y-reset-btn').addEventListener('click',function(){resetAll();});
      function resetAll(silent){
        const classes=['a11y-high-contrast','a11y-dark','a11y-grayscale','a11y-negative','a11y-dyslexia','a11y-highlight-links','a11y-read-guide','a11y-letter-spacing','a11y-big-cursor','a11y-focus-visible','a11y-no-images','a11y-no-motion','a11y-keyboard'];
        classes.forEach(function(c){html.classList.remove(c);});
        html.style.fontSize='';S.fontIdx=2;
        if(fontDisplay)fontDisplay.textContent='100%';
        Object.keys(TOGGLES).forEach(function(id){const toggle=document.getElementById(id);const row=document.getElementById('lbl-'+id);if(toggle){toggle.checked=false;toggle.setAttribute('aria-checked','false');}if(row)row.classList.remove('on');});
        document.querySelectorAll('.a11y-color-opt').forEach(function(b){b.classList.remove('active');b.setAttribute('aria-pressed','false');});
        const normalBtn=document.querySelector('.a11y-color-opt[data-mode="normal"]');
        if(normalBtn){normalBtn.classList.add('active');normalBtn.setAttribute('aria-pressed','true');}
        document.querySelectorAll('.a11y-profile').forEach(function(b){b.classList.remove('active');b.setAttribute('aria-pressed','false');});
        if(readGuide)readGuide.style.display='none';
        S.colorMode='normal';
        if(!silent){announce('All settings reset');save();}
      }
      function announce(msg){if(!announcer)return;announcer.textContent='';requestAnimationFrame(function(){announcer.textContent=msg;});}
      function save(){
        try{const data={fontIdx:S.fontIdx,colorMode:S.colorMode,toggles:{}};Object.keys(TOGGLES).forEach(function(id){const toggle=document.getElementById(id);data.toggles[id]=toggle&&toggle.checked;});localStorage.setItem(STORAGE_KEY,JSON.stringify(data));}catch(e){}
      }
      function restore(){
        try{const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return;const data=JSON.parse(raw);if(data.fontIdx!==undefined){S.fontIdx=data.fontIdx;applyFont();}if(data.colorMode&&data.colorMode!=='normal'){const modeBtn=document.querySelector('.a11y-color-opt[data-mode="'+data.colorMode+'"]');if(modeBtn){document.querySelectorAll('.a11y-color-opt').forEach(function(b){b.classList.remove('active');b.setAttribute('aria-pressed','false');});modeBtn.classList.add('active');modeBtn.setAttribute('aria-pressed','true');['high-contrast','dark','grayscale','negative'].forEach(function(m){html.classList.remove('a11y-'+m);});html.classList.add('a11y-'+data.colorMode);S.colorMode=data.colorMode;}}if(data.toggles){Object.keys(data.toggles).forEach(function(id){if(data.toggles[id]){const toggle=document.getElementById(id);const row=document.getElementById('lbl-'+id);if(toggle){toggle.checked=true;toggle.setAttribute('aria-checked','true');if(row)row.classList.add('on');html.classList.add(TOGGLES[id].cls);if(id==='t-readguide')readGuide.style.display='block';}}});}}catch(e){}
      }
      panel.addEventListener('keydown',function(e){
        if(e.key!=='Tab')return;
        const focusable=Array.from(panel.querySelectorAll('button,input:not([tabindex="-1"]),select,[tabindex]:not([tabindex="-1"])'));
      ...
      restore();
    }catch(err){console.error('Accessibility toolbar error:',err);}
  },100);
});
